...
| Git | 版本控制 |
| Claude Code | AI 交互界面 |
| gh CLI | 可选，用于 GitHub 操作 |

> **不需要预装 Python**——uv 会自动按 `pyproject.toml` 装好正确版本。
...
重启 Claude Code，输入 `/team` 能看到提示就装好了。

> 第一次启动 MCP server 时 `uvx` 会自动从 GitHub 拉取代码、装依赖、跑起来；之后秒启。
> **新手用户**：详细的队长 / 队员操作流程见 [docs/USAGE.md](docs/USAGE.md)。
